...
$('a').click(function(){
    ...
    action=1;
        ...
        ZOHO.CRM.CONFIG.getOrgVariable("message360sruthy.accSID").then(function(data) {
            utils.accntSID = data.Success.Content;
        }).then(function() {
            ZOHO.CRM.CONFIG.getOrgVariable("message360sruthy.authToken").then(function(data) {
                ...
                request = {
                    url: "https://api.message360.com/api/v2/incomingphone/listnumber.json",
                    params: {
                ...
                ZOHO.CRM.API.getAllRecords({
                        Entity: "message360sruthy.SMSTemplates",
                        sort_order: "desc"
                    })
                    .then(function(data) {
                            s = {
...
function back()
{
    ...
    ZOHO.CRM.API.getAllRecords({
        ...
        .then(function(data) {
           ...
           txt = (data["message360sruthy.Template_Content"]);
    ...
    else if (template.content=="")
{
   ...
   $("#templateEmptyAlert").show();
    ...
    if(template.name!=""&&template.content!="")
    {
         $('#loading').show();
        var recordData = {
            "message360sruthy.CustomModule1_Name": template.name,
            "message360sruthy.Template_Content": template.content
        }
...
function sendsms() {
    ...
    if (recepients) {
        ...
        for (var i in recepients) {
            ...
            request = {
                url: "https://api.message360.com/api/v2/sms/sendsms.json",
                params: {
                    AccountSid: utils.accntSID,
                    fromcountrycode: "1",
                    tocountrycode: "+91",
...
function showReport(data, messages, frmNo) {
    ...
    for (var index in recepients) {
        ...
        var recData = {
            "message360sruthy.CustomModule2_Name": recepients[index].data.Full_Name,
            "message360sruthy.Message_Sent": messages[index],
            "message360sruthy.Status": status,
            "message360sruthy.Sent_From": frmNo,
            "message360sruthy.Contact_Name": recepients[index].data.id,
            "message360sruthy.Template_Used":utils.id
        }
        ZOHO.CRM.API.insertRecord({
            Entity: "message360sruthy.Message360_SMS_History",
            APIData: recData
        }).then(function(data) {
        });
